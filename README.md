...
## Architecture
- **Ingestion**: PDF upload → pypdf parsing → chunking → Vertex AI embeddings → upsert to Vertex AI Vector Search with metadata.
- **Retrieval QA**: Paper-specific queries fetch relevant chunks from Vector Search, and Gemini generates grounded answers.
- **State**: Firestore stores chat history and running summaries.
- **Storage**: Raw PDFs live in Cloud Storage; summaries are persisted in Firestore.

### Key components
- `ingestion/pipeline.py` — PDF → chunks → embeddings → Vertex AI Vector Search + summary generation.
- `agents/adk_agent.py` — ADK-style agent that retrieves from Vector Search and calls Gemini.
...
- `services/storage.py` — Firestore-backed summaries and chat history.
...
4. **Index**: Upsert embeddings + metadata into Vertex AI Vector Search using `paper_id` namespace filters.
